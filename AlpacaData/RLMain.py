...
VAR_N_TOTAL_EPISODES = 200 #900 # 200
VAR_TESTING_POINTS = 10 # 50 # 10
VAR_SEED_SHUFFLE = 3
VAR_SEED_AGENT = 3
VAR_ID_PATH = '000003'
...
VAR_SHUFFLE_DAYS = True

# HYPERPARAMETERS
HP_CORRELATION_PUNISHMENT = 1
HP_REWARD_TYPE = 'sharpewithcorrelation' # 'sharpe', 'sharpewithcorrelation', 'netprofit'
HP_RETURN_MEMORY_SIZE = 5
...
from AgentsToUse import PPOAgent, A2CAgent, DDPGAgent, RandomAgent, SP500Agent
...
def main(SHUFFLE_DAYS=VAR_SHUFFLE_DAYS,
         ...
         CORRELATION_PUNISHMENT=HP_CORRELATION_PUNISHMENT, 
         REWARD_TYPE=HP_REWARD_TYPE, 
         RETURN_MEMORY_SIZE=HP_RETURN_MEMORY_SIZE):
    # Setting Metadata
    metadata = {}
    ...
    # Getting data
    training_data = get_data(tickers)
    ...
    env, ppo_agent, a2c_agent, ddpg_agent, random_agent , sp500_agent = create_env_and_train_agents(datasetToTrain, benchmarkToTrain, n_steps=n_steps, return_memory_size=RETURN_MEMORY_SIZE, reward_type=REWARD_TYPE, env_correlation_punishment=CORRELATION_PUNISHMENT, env_initial_balance=INITIAL_BALANCE, seed=SEED_AGENT)
    ...
    exportMetricsResume(metrics_resume, metrics_resume_testing, training_rewards_resume, metadata, trained_agents , path_to_save = PATH_TO_SAVE)
...
def create_env_and_train_agents(data, benchmark_data, n_steps, return_memory_size, reward_type, env_correlation_punishment, env_initial_balance, seed):
    ...
    # Train PPO Agent
    ppo_agent = PPOAgent(env, n_steps=n_steps, seed=seed)
    # Train A2C Agent
    a2c_agent = A2CAgent(env, n_steps=n_steps, seed=seed)
    # Train DDPG Agent
    ddpg_agent = DDPGAgent(env, seed=seed)
    # Declaring Random Agent
    random_agent = RandomAgent(env)
    # Declaring SP500 Agent
    sp500_agent = SP500Agent(env)
    return env, ppo_agent, a2c_agent, ddpg_agent, random_agent, sp500_agent 
